...
var toState = function toState(x) {
  ...
  if (x instanceof BigInt64Array) return uint8ArrayToHexString(x).map(function (x) {
    return x.toString();
  });
  if (x instanceof BigUint64Array) return uint8ArrayToHexString(x).map(function (x) {
    return x.toString();
  });
