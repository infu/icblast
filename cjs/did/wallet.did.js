...
var idlFactory = function idlFactory(_ref) {
  ...
  var Role = IDL.Variant({
    Custodian: IDL.Null,
    Contact: IDL.Null,
    Controller: IDL.Null
  });
  var AddressEntry = IDL.Record({
    id: IDL.Principal,
    kind: Kind,
    name: IDL.Opt(IDL.Text),
    role: Role
  });
  var WalletResult = IDL.Variant({
    Ok: IDL.Null,
    Err: IDL.Text
  });
  var EventKind = IDL.Variant({
  ...
  var WalletResultCall = IDL.Variant({
    Ok: IDL.Record({
      "return": IDL.Vec(IDL.Nat8)
    }),
    Err: IDL.Text
  });
  var CanisterSettings = IDL.Record({
    controller: IDL.Opt(IDL.Principal),
    freezing_threshold: IDL.Opt(IDL.Nat),
    memory_allocation: IDL.Opt(IDL.Nat),
    compute_allocation: IDL.Opt(IDL.Nat)
  });
  var CreateCanisterArgs = IDL.Record({
    cycles: IDL.Nat64,
    settings: CanisterSettings
  });
  var WalletResultCreate = IDL.Variant({
    Ok: IDL.Record({
  ...
  return IDL.Service({
    ...
    get_chart: IDL.Func([IDL.Opt(IDL.Record({
    ...
    get_events: IDL.Func([IDL.Opt(IDL.Record({
    ...
    wallet_create_canister: IDL.Func([CreateCanisterArgs], [WalletResultCreate], []),
    wallet_create_wallet: IDL.Func([CreateCanisterArgs], [WalletResultCreate], []),
    wallet_receive: IDL.Func([], [], []),
    wallet_send: IDL.Func([IDL.Record({
      canister: IDL.Principal,
      amount: IDL.Nat64
    })], [WalletResult], []),
    wallet_store_wallet_wasm: IDL.Func([IDL.Record({
