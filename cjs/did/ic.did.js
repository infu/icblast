...
var idlFactory = function idlFactory(_ref) {
  ...
  var get_balance_request = IDL.Record({
    network: bitcoin_network,
    address: bitcoin_address,
    min_confirmations: IDL.Opt(IDL.Nat32)
  });
  var satoshi = IDL.Nat64;
  var get_current_fee_percentiles_request = IDL.Record({
    network: bitcoin_network
  });
  var millisatoshi_per_byte = IDL.Nat64;
  var get_utxos_request = IDL.Record({
    network: bitcoin_network,
  ...
  var block_hash = IDL.Vec(IDL.Nat8);
  ...
  var get_utxos_response = IDL.Record({
    next_page: IDL.Opt(IDL.Vec(IDL.Nat8)),
    tip_height: IDL.Nat32,
    tip_block_hash: block_hash,
    utxos: IDL.Vec(utxo)
  });
  var send_transaction_request = IDL.Record({
  ...
  var http_response = IDL.Record({
    status: IDL.Nat,
    body: IDL.Vec(IDL.Nat8),
    headers: IDL.Vec(http_header)
  });
  var wasm_module = IDL.Vec(IDL.Nat8);
  var vetkd_curve = IDL.Variant({
    bls12_381: IDL.Null
  });
  return IDL.Service({
    ...
    ecdsa_public_key: IDL.Func([IDL.Record({
      key_id: IDL.Record({
        name: IDL.Text,
        curve: ecdsa_curve
      }),
    ...
    sign_with_ecdsa: IDL.Func([IDL.Record({
      key_id: IDL.Record({
        name: IDL.Text,
        curve: ecdsa_curve
      }),
    ...
    vetkd_encrypted_key: IDL.Func([IDL.Record({
      key_id: IDL.Record({
        name: IDL.Text,
        curve: vetkd_curve
      }),
    ...
    vetkd_public_key: IDL.Func([IDL.Record({
      key_id: IDL.Record({
        name: IDL.Text,
        curve: vetkd_curve
      }),
