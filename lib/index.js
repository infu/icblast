import {
  ic,
  hashIdentity,
  toState,
  explainMethodSchema,
  explainServiceSchema,
  validateMethodInput,
  validateMethodInputSchema,
  idlFactoryFromCandid,
} from './icb_node.js';
import { loadExistingIdentity } from './identity.js';
import { loadSchemaCache, saveSchemaCache } from './cache.js';
import Ajv2020 from 'ajv/dist/2020.js';
import { defineDataProperty, hasOwn } from './candid_object.js';

async function getActor(
  canister,
  {
    host,
    id = 0,
    debug = false,
    agentOptions,
    maxHttpResponseBytes,
    maxDecodedCandidItems,
    maxDecodedCandidDepth,
    maxCandidTypeItems,
    maxCandidTypeDepth,
    allowNumberedPrincipals = true,
  } = {},
) {
  const iden = await hashIdentity(id);
  const getIC = await ic({
    identity: iden,
    host,
    idNum: id,
    debug,
    agentOptions,
    maxHttpResponseBytes,
    maxDecodedCandidItems,
    maxDecodedCandidDepth,
    maxCandidTypeItems,
    maxCandidTypeDepth,
    allowNumberedPrincipals,
  });
  return await getIC(canister);
}

async function principal(id = 0) {
  const iden = await hashIdentity(id);
  return iden.getPrincipal().toText();
}

async function scan(canister, opts = {}) {
  const actor = await getActor(canister, opts);
  const { IDL } = await import('@dfinity/candid');
  const service = actor.$idlFactory({ IDL });
  const methods = [...service._fields].map(([n, f]) => {
    const ann = (f && f.annotations) || [];
    let kind = 'update';
    if (ann.includes('query') || ann.includes('composite_query')) kind = 'query';
    else if (ann.includes('oneway')) kind = 'oneway';
    return { name: n, kind };
  });
  // refresh schema cache best-effort
  try {
    const schemaMap = {};
    for (const m of methods) {
      try {
        defineDataProperty(
          schemaMap,
          m.name,
          explainMethodSchema(actor, m.name),
        );
      } catch {}
    }
    await saveSchemaCache(canister, { canister, updatedAt: new Date().toISOString(), methods: schemaMap });
  } catch {}
  return methods;
}

async function schema(canister, method, opts = {}) {
  const cache = opts.allowNumberedPrincipals === false
    ? null
    : await loadSchemaCache(canister);
  if (hasOwn(cache?.methods, method)) return cache.methods[method];
  const actor = await getActor(canister, opts);
  return explainMethodSchema(actor, method, opts);
}

function actorMethod(actor, method) {
  return actor.$methods?.get?.(method) ??
    (hasOwn(actor, method) ? actor[method] : undefined);
}

async function call(canister, method, args = [], opts = {}) {
  const actor = await getActor(canister, opts);
  const fn = actorMethod(actor, method);
  if (typeof fn !== 'function') throw new Error(`Method not found: ${method}`);
  const out = await fn(...args);
  return toState(out);
}

async function validate(canister, method, args = [], opts = {}) {
  const actor = await getActor(canister, opts);
  const cache = opts.allowNumberedPrincipals === false
    ? null
    : await loadSchemaCache(canister);
  const sch = hasOwn(cache?.methods, method)
    ? cache.methods[method]
    : explainMethodSchema(actor, method, opts);
  const input = validateMethodInputSchema(sch, args);
  if (!input.ok) return { ok: false, inputValid: false, outputValid: false, errors: input.errors };
  const fn = actorMethod(actor, method);
  if (typeof fn !== 'function') throw new Error(`Method not found: ${method}`);
  const out = await fn(...args);
  const outNorm = toState(out);
  const ajv = new Ajv2020({ allErrors: true, ownProperties: true, strict: false });
  const vout = ajv.compile(sch.output);
  const outOk = vout(outNorm);
  return { ok: outOk, inputValid: true, outputValid: outOk, errors: outOk ? undefined : vout.errors };
}

const api = {
  principal,
  scan,
  schema,
  call,
  validate,
  ic: async (opts = {}) => {
    const iden = await hashIdentity(opts.id ?? 0);
    return ic({
      identity: iden,
      host: opts.host,
      idNum: opts.id ?? 0,
      debug: opts.debug,
      agentOptions: opts.agentOptions,
      maxHttpResponseBytes: opts.maxHttpResponseBytes,
      maxDecodedCandidItems: opts.maxDecodedCandidItems,
      maxDecodedCandidDepth: opts.maxDecodedCandidDepth,
      maxCandidTypeItems: opts.maxCandidTypeItems,
      maxCandidTypeDepth: opts.maxCandidTypeDepth,
      allowNumberedPrincipals: opts.allowNumberedPrincipals,
    });
  },
  // re-export useful utilities
  hashIdentity,
  loadExistingIdentity,
  toState,
  explainMethodSchema,
  explainServiceSchema,
  validateMethodInput,
  validateMethodInputSchema,
  idlFactoryFromCandid,
};

export {
  ic,
  hashIdentity,
  loadExistingIdentity,
  toState,
  explainMethodSchema,
  explainServiceSchema,
  validateMethodInput,
  validateMethodInputSchema,
  idlFactoryFromCandid,
};

export default api;
