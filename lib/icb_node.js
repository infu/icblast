...
function schemaOfType(t, options = {}) {
  ...
  if (t instanceof xVec) {
    ...
    return { type: "array", items: schemaOfType(t.val, options) };
  }
  if (t instanceof xTuple) {
    const items = t.val.map((item) => schemaOfType(item, options));
  ...
  if (t instanceof xRecord) {
    ...
    for (const k of Object.keys(t.val)) {
      ...
      props[k] = schemaOfType(isOpt ? inner.val : inner, options);
  ...
  if (t instanceof xOpt) { const inner = schemaOfType(t.val, options); return { anyOf: [ inner, { type: "null" } ] }; }
  if (t instanceof xVariant) {
    const alts = [];
    for (const tag of Object.keys(t.val)) {
      const inner = schemaOfType(t.val[tag], options);
...
function schemaFromSignature(sig, options = {}) {
  ...
  const inputPrefix = args.map((type) => schemaOfType(type, options));
  ...
  else if (outs.length === 1) { const t = outs[0]; output = isResultVariant(t) ? schemaOfType(t.val.Ok ?? t.val.ok, options) : schemaOfType(t, options); output.$schema = "https://json-schema.org/draft/2020-12/schema"; }
  else output = { $schema: "https://json-schema.org/draft/2020-12/schema", type: "array", prefixItems: outs.map((type) => schemaOfType(type, options)), minItems: outs.length, maxItems: outs.length };
  return { input, output };
}
...
async function didToJsBindings(did) {
  const maxCandidSourceBytes = 128 * 1024;
  const maxGeneratedJavaScriptBytes = 2 * 1024 * 1024;
  if (typeof did !== "string" || !did.trim()) {
    throw new Error("Candid interface must be a non-empty string");
  }
  if (Buffer.byteLength(did, "utf8") > maxCandidSourceBytes) {
    throw new Error(
  ...
  try {
    ...
    if (Buffer.byteLength(js, "utf8") > maxGeneratedJavaScriptBytes) {
      throw new Error(
...
// ========== Client ==========
export async function ic({ identity, host = "https://icp0.io", idNum = 0, debug = false, allowNumberedPrincipals = true } = {}) {
