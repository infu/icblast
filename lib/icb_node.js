...
async function ensureSecretString() {
  ...
  try {
    ...
    if (cleaned.length >= 64) return cleaned; // minimal length for our file secret
  } catch (_) {
...
function deriveSeedFromSecret(secretStr, idNum) {
  ...
  const twice = secretStr + secretStr;
...
export async function hashIdentity(idNum = 0) {
  ...
  const seed = deriveSeedFromSecret(secretSource, idNum);
...
function isIcrcAccountDef(d) {
  ...
  if (!Object.prototype.hasOwnProperty.call(v, "subaccount")) return true; // owner-only is acceptable
...
function convertBack(input, def, ctx) {
