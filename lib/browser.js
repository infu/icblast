...
import initDidcWasm, {
  generate as generateDidcBindings,
...
function schemaOfType(type, options = {}) {
  ...
  if (type instanceof xVec) {
    ...
    return { type: "array", items: schemaOfType(type.val, options) };
  }
  if (type instanceof xTuple) {
    const items = type.val.map((item) => schemaOfType(item, options));
    return {
      type: "array",
      prefixItems: items,
  ...
  if (type instanceof xRecord) {
    ...
    for (const key of Object.keys(type.val)) {
      ...
      properties[key] = schemaOfType(optional ? inner.val : inner, options);
  ...
  if (type instanceof xOpt) {
    return { anyOf: [schemaOfType(type.val, options), { type: "null" }] };
  }
  if (type instanceof xVariant) {
    return {
      oneOf: Object.keys(type.val).map((key) => ({
        type: "object",
        properties: { [key]: schemaOfType(type.val[key], options) },
        required: [key],
...
function schemaFromSignature(signature, options = {}) {
  const inputPrefix = signature.input.map((type) => schemaOfType(type, options));
  ...
  if (signature.output.length === 0) {
    ...
    output = isResultVariant(type)
      ? schemaOfType(okType, options)
      : schemaOfType(type, options);
    output.$schema = "https://json-schema.org/draft/2020-12/schema";
  } else {
    output = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      type: "array",
      prefixItems: signature.output.map((type) => schemaOfType(type, options)),
...
export function explainServiceSchema(source, options = {}) {
  ...
  for (const method of Object.keys(xdl)) {
    schemas[method] = schemaFromSignature(xdl[method], options);
  }
  return schemas;
}
...
async function ensureDidcWasm(source) {
  if (!didcWasmReady) {
    didcWasmReady = initDidcWasm({ module_or_path: source });
  }
...
async function didToJs(did, wasmSource = defaultDidcWasmSource) {
  ...
  if (candidBytes > MAX_CANDID_SOURCE_BYTES) {
    throw new Error(
      `Candid interface exceeds ${MAX_CANDID_SOURCE_BYTES} UTF-8 bytes`,
    );
  }
  await ensureDidcWasm(wasmSource);
  const bindings = generateDidcBindings(did);
  if (!bindings) throw new Error("Unable to generate JS bindings from Candid");
  try {
    const js = bindings.js;
    if (typeof js !== "string" || !js.trim()) {
      throw new Error("Unable to generate JS bindings from Candid");
    }
    if (
      new TextEncoder().encode(js).byteLength > MAX_GENERATED_JAVASCRIPT_BYTES
    ) {
      throw new Error(
...
export async function idlFactoryFromCandid(
  did,
  { didcWasm = defaultDidcWasmSource } = {}
) {
  return evalIdlFactory(await didToJs(did, didcWasm));
}
